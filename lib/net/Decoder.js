...
function Decoder(){
  ...
  this.DecodePcap=function DecodePacket(packet){//initalize the protocol structure.
    var proto=MakeFrame(packet);
    this.Decode(proto);
    return proto;
  };
  ...
  this._transform=function Transform(chunk,encoding,next){
    if(chunk['constructor']!==undefined&&chunk.constructor.name==='PacketWithHeader') //pcap packet
      chunk=MakeFrame(chunk);
